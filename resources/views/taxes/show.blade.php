        ...
        <x-slot:tbody>
            @foreach($logs as $log)
                <tr>
                    ...
                    <td><strong>Всего: {{ rtrim(rtrim(number_format($log->coins_total, 4, ',', ' '), '\0'), '\,') }}</strong><br><x-moonshine::badge color="yellow">{{ now()->translatedFormat('M') }}: {{ rtrim(rtrim(number_format($log->coins_current_month, 4, ',', ' '), '\0'), '\,') }}<br>{{ now()->subMonth()->translatedFormat('M') }}: {{ rtrim(rtrim(number_format($log->coins_previous_month, 4, ',', ' '), '\0'), '\,') }}<br>{{ now()->subMonth(2)->translatedFormat('M') }}: {{ rtrim(rtrim(number_format($log->coins_two_months_ago, 4, ',', ' '), '\0'), '\,') }}</x-moonshine::badge></td>
